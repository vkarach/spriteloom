...
showHistory = function(client)
  ...
  local thumbsList = {}     -- decoded as pages arrive: scroll stays free
  local thumbsGrid = {}     -- separately fitted since the grid box differs
  local status = "Loading history..."
  local ROWH, LISTW, VIS = 56, 600, 7
  local COLS, GW, GH, GVIS = 3, LISTW // 3, 140, 3
  -- Prefetch ~3 screens ahead so opening History doesn't wait on the archive.
  local PAGE = 21
  local nextOffset, fetching, allLoaded = 0, false, false

  -- Canvas height is fixed at creation, so each mode gets its own dialog.
  local function open()
    local scroll = 0
    local LISTH = (histMode == "grid") and (GH * GVIS) or (ROWH * VIS)
    ...
    fetchMore = function()
      ...
      client.history(nextOffset, PAGE, true, function(msg)
        ...
        for _, run in ipairs(msg.runs) do
          rows[#rows + 1] = run
          local raw = sprite.imageFromPayload(run.images[1], "t" .. #rows)
          thumbsList[#rows] = ui.scaleToFit(raw, ROWH - 8, ROWH - 8)
          thumbsGrid[#rows] = ui.scaleToFit(raw, GW - 8, GH - 8)
        end
        fetching = false
      ...
      onpaint = function(ev)
        ...
        if histMode == "grid" then
          for v = 0, GVIS - 1 do
            for c = 0, COLS - 1 do
              ...
              if rows[i] then
                ...
                ui.drawThumb(gc, thumbsGrid[i], x + 4, y + 4, GW - 8, GH - 8)
              end
            end
          end
        else
          for v = 1, VIS do
            ...
            ui.drawThumb(gc, thumbsList[i], 4, y + 4, ROWH - 8, ROWH - 8)
            gc.color = text
